...
echo "==> 1/4 Форматирование и анализ"
./tools/check.sh format
./tools/check.sh analyze

echo
echo "==> 2/4 Тесты приложений и пакетов"
./tools/check.sh test

echo
echo "==> 3/4 Прогон полного дня и остальные тесты сервера"
(cd server && dart test)

echo
echo "==> 4/4 Релизные APK (подписанные, по одному на архитектуру)"
